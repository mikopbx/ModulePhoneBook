<?php
return [
    'module_phnbk_Disconnected' => 'Η μονάδα απενεργοποιήθηκε',
    'module_phnbk_Connected' => 'Η μονάδα είναι συνδεδεμένη',
    'module_phnbk_AlreadyExistWithThisNumber' => 'Ο αριθμός καταχώρισης %repesent% υπάρχει ήδη',
    'module_phnbk_AddNewRecord' => 'Προσθήκη νέας καταχώρισης',
    'module_phnbk_ColumnNumber' => 'Τηλέφωνο',
    'module_phnbk_ColumnName' => 'Όνομα συνδρομητή',
    'SubHeaderModulePhoneBook' => 'Σας επιτρέπει να εμφανίζετε τα ονόματα των συνδρομητών κατά τη διάρκεια μιας κλήσης και στο ιστορικό των συνομιλιών',
    'BreadcrumbModulePhoneBook' => 'Τηλεφωνικό κατάλογο',
    'mo_ModuleModulePhoneBook' => 'Μονάδα τηλεφωνικού καταλόγου',
    /**
 * Copyright (C) MIKO LLC - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 * Written by Nikolay Beketov, 6 2018
 *
 */
    'repModulePhoneBook' => 'Μονάδα τηλεφωνικού καταλόγου - %repesent%',
    'module_phnbk_Search' => 'Εύρημα...',
    'module_phnbk_ImportFromExcel' => 'Εισαγωγή',
    'module_phnbk_ImportExcelTab' => 'Εισαγωγή από το Excel',
    'module_phnbk_PhonebookTab' => 'Τηλεφωνικός κατάλογος',
    'module_phnbk_ImportFromExcelLabel' => 'Επιλέξτε αρχείο Excel για λήψη',
    'module_phnbk_ExcelInstructionStep4' => 'Βεβαιωθείτε ότι τα δεδομένα στο αρχείο είναι σωστά πριν το ανεβάσετε.',
    'module_phnbk_ImportError' => 'Σφάλμα κατά την αποθήκευση της καταχώρισης',
    'module_phnbk_SettingsTab' => 'Ρυθμίσεις',
    'module_phnbk_ExcelInstructionHeader' => 'Οδηγίες για τη λήψη ενός τηλεφωνικού καταλόγου από το Excel',
    'module_phnbk_ExcelInstructionStep1' => 'Το αρχείο πρέπει να έχει τη μορφή <strong>.xls</strong> ή <strong>.xlsx.</strong>',
    'module_phnbk_ExcelInstructionStep2' => 'Το αρχείο πρέπει να περιέχει δύο στήλες:',
    'module_phnbk_ExcelInstructionStep2_1' => '<strong>CallerID</strong> - όνομα συνδρομητή (για παράδειγμα: Ivan Ivanov).',
    'module_phnbk_ExcelInstructionStep2_2' => '<strong>Αριθμός τηλεφώνου</strong> - σε μορφή <em>+7(926)123-45-67</em>.',
    'module_phnbk_ExcelInstructionStep3' => 'Κάθε γραμμή αντιπροσωπεύει μια καταχώρηση τηλεφωνικού καταλόγου.',
    'module_phnbk_disableInputMask' => 'Απενεργοποιήστε τη μάσκα αριθμού',
    'module_phnbk_DeleteAllRecords' => 'Εκκαθάριση τηλεφωνικού καταλόγου',
    'module_phnbk_DeleteAllDescription' => 'Όλες οι καταχωρήσεις τηλεφωνικού καταλόγου θα διαγραφούν οριστικά, εάν χρειαστεί να διαγράψετε μία ή μια καταχώρηση, χρησιμοποιήστε το κουμπί στον πίνακα.',
    'module_phnbk_GeneraLFileUploadError' => 'Σφάλμα κατά τη μεταφόρτωση του αρχείου',
    'module_phnbk_UploadError' => 'Σφάλμα λήψης αρχείου',
    'module_phnbk_UploadInProgress' => 'Μεταφόρτωση αρχείου στον διακομιστή',
    'module_phnbk_NoFileUploaded' => 'Το αρχείο εισαγωγής δεν φορτώθηκε',
    'module_phnbk_invalidFormat' => 'Σφάλμα μορφής αρχείου',
    'module_phnbk_DeleteAllTitle' => 'Προσοχή!',
    'module_phnbk_CancelBtn' => 'Ματαίωση',
    'module_phnbk_Approve' => 'Διαγράψτε τα πάντα',
    'module_phnbk_AllRecordsDeleted' => 'Όλες οι καταχωρήσεις έχουν διαγραφεί',
    'module_phnbk_RecognitionOnProgress' => 'Ανάλυση και φόρτωση δεδομένων από ένα αρχείο',
    'module_phnbk_RecognitionFinished' => 'Η φόρτωση δεδομένων ολοκληρώθηκε',
];
